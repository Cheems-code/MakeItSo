...
fun TasksScreen(
  ...
  // Recoge el flujo de tareas como un estado usando `collectAsStateWithLifecycle`
  val tasks = viewModel
    .tasks
    .collectAsStateWithLifecycle(emptyList())

  // Llama a `TasksScreenContent` pasando el estado de las tareas
  TasksScreenContent(
    tasks = tasks.value,
  ...
  // Carga las opciones de tareas al iniciarse el ViewModel
...
fun TasksScreenContent(
  tasks: List<Task>, // Nueva lista de tareas como parámetro
  ...
  Scaffold(
    ...
    Column(modifier = Modifier.fillMaxWidth().fillMaxHeight()) {
      ...
      // `LazyColumn` utiliza la lista de tareas pasada como parámetro
      LazyColumn {
        items(tasks, key = { it.id }) { taskItem ->
          TaskItem(
            task = taskItem,
            options = listOf(), // Este podría ser viewModel.options.value si está implementado
